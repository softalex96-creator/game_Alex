...
export async function verifyFirebaseIdToken(idToken, fetchImpl = fetch, requireVerified = true) {
  ...
  if (!response.ok || !user?.localId || !user?.email || (requireVerified && user.emailVerified !== true)) throw new Error("Firebase user is not verified");
  return { uid: user.localId, email: user.email, displayName: user.displayName || "Игрок LevelUp" };
}

export async function authenticateFirebaseRequest(request, fetchImpl = fetch, requireVerified = true) {
  const authorization = String(request.headers.authorization || "");
  const match = authorization.match(/^Bearer\s+(.+)$/i);
  if (!match) throw new Error("Authorization is required");
  return verifyFirebaseIdToken(match[1], fetchImpl, requireVerified);
}
