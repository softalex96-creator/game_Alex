...
async function loadServerOrders(user) {
  try { const response = await fetch("https://api.gamemaster.cc/admin/orders", { headers: { Authorization: `Bearer ${await user.getIdToken()}` }, cache: "no-store" }); if (!response.ok) throw new Error("request"); const payload = await response.json(); if (Array.isArray(payload.orders)) state.orders = payload.orders; } catch { /* Keep the local operator queue available if API orders are unavailable. */ }
}
...
async function renderUsers(user) {
  const content = document.querySelector(".admin-content");
  if (!content || document.querySelector("[data-admin-users]")) return;
  const panel = document.createElement("section"); panel.className = "admin-panel admin-users"; panel.dataset.adminUsers = "true";
  panel.innerHTML = `<div class="admin-panel__heading"><div><p class="admin-kicker">АККАУНТЫ</p><h2>Игроки LevelUp</h2></div><span data-admin-user-count>—</span></div><div class="admin-user-list" data-admin-user-list><p class="admin-empty">Загружаем аккаунты…</p></div>`; content.append(panel);
  try { const response = await fetch("https://api.gamemaster.cc/admin/users", { headers: { Authorization: `Bearer ${await user.getIdToken()}` }, cache: "no-store" }); if (!response.ok) throw new Error("request"); const { users = [] } = await response.json(); const list = panel.querySelector("[data-admin-user-list]"); panel.querySelector("[data-admin-user-count]").textContent = String(users.length); list.replaceChildren(); if (!users.length) { list.innerHTML = `<p class="admin-empty"><strong>Пока никого нет</strong>Зарегистрированные игроки появятся после первого входа.</p>`; return; } users.forEach((player) => { const item = document.createElement("article"); item.className = "admin-user-row"; item.innerHTML = `<div><strong></strong><small></small></div><time></time>`; item.querySelector("strong").textContent = player.displayName || "Игрок LevelUp"; item.querySelector("small").textContent = `${player.email} · ${player.uid}`; item.querySelector("time").textContent = player.createdAt ? new Intl.DateTimeFormat("ru-RU", { day: "numeric", month: "short", year: "numeric" }).format(new Date(player.createdAt)) : "—"; list.append(item); }); } catch { panel.querySelector("[data-admin-user-list]").innerHTML = `<p class="admin-empty">Не удалось загрузить список аккаунтов.</p>`; }
}
